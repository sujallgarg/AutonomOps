...
export function UserProfileDropdown({ session, onOpenAuthModal }: UserProfileDropdownProps) {
  ...
  return (
    <div className="relative font-sans z-50" ref={dropdownRef}>
      {/* Top-Right Corner Premium Profile Pill Button with Circular Avatar */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="group relative flex items-center space-x-2.5 rounded-full p-1 pr-3 text-xs text-slate-100 bg-[#090d19]/90 border border-slate-800 hover:border-slate-700 transition-all duration-200 cursor-pointer shadow-lg backdrop-blur-xl hover:bg-slate-900/90 active:scale-98"
      >
        {/* Screenshot Style Circular Avatar Badge */}
        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#dbe1ea] text-black font-extrabold text-xs tracking-tight shadow-md border-2 border-slate-900 group-hover:scale-105 transition-transform shrink-0">
          {getInitials(session.name)}
        </div>

        {/* User Name, Role & Trial Pill Subtext */}
      ...
      {/* Sleek Interactive Dropdown Menu Matching Screenshot Design */}
      {isOpen && (
        <div className="absolute right-0 mt-3 w-80 rounded-3xl border border-slate-800/90 bg-[#070a14] p-5 shadow-2xl z-50 text-xs space-y-4 text-slate-200 backdrop-blur-2xl animate-in fade-in slide-in-from-top-2 font-sans">
          {/* Header User Card (Avatar + Name + Email + Role) */}
          <div className="flex items-center space-x-3 border-b border-slate-800/80 pb-4">
            <div className="flex h-11 w-11 items-center justify-center rounded-full font-extrabold text-black bg-[#dbe1ea] text-sm shadow-md border-2 border-slate-900 shrink-0">
              {getInitials(session.name)}
            </div>

            <div className="overflow-hidden flex-1 space-y-0.5">
              <h4 className="font-extrabold text-white text-base truncate tracking-tight">{session.name}</h4>
          ...
          {/* ACTIVE PLAN & 15-DAY FREE TRIAL CARD */}
          <div className="rounded-2xl border border-slate-800/90 bg-[#0b0f1d] p-4 font-sans space-y-2 shadow-inner">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400 font-medium">Active Plan:</span>
              <span className="bg-[#161c2d] text-emerald-400 font-bold px-3 py-1 rounded-xl text-xs border border-emerald-500/30 shadow-sm flex items-center gap-1.5">
          ...
          {!isSubscribed ? (
            <button
              ...
              className="w-full rounded-2xl bg-white hover:bg-slate-100 text-black font-extrabold text-sm py-3 px-4 shadow-xl transition-all duration-200 flex items-center justify-between cursor-pointer group active:scale-98"
            >
